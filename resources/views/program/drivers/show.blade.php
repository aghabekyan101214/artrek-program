                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        @foreach($months as $bin => $month)
                            <tr onclick="openMonth('month-{{ $bin }}')" style="cursor: pointer">
                                <td>{{ $month['name'] }}</td>
                                <td><span style="font-size: 16px;" class="badge badge-success ">{{ $driver->paidSalary->where('month', $month['index'])->where('year', Request::get("year"))->sum('price') * - 1 }}</span></td>
                            </tr>
                            @foreach($driver->paidSalary->where('month', $month['index'])->where('year', Request::get("year")) as $s)
                                    ...
                                    <td colspan="2" style="background: white; padding: 0 0 0 20px">
                                        <table style="width: 50%; background: white">
                                            <tr style="background: white">
                                                <td style="background: white; width: 50%"><b>{{ $s->price * -1 }}</b>{{ " - " . $s->created_at . " - " . ($s->type ? "Փոխանցում" : "Կանխիկ") }}</td>
                                                <td>
